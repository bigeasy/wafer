...
sources = docco/wafer.html css/style.css index.html
...
watch: all
	fswatch --exclude '.' --include '\.html$$' --include '\.less$$' --include '\.md$$' --include '\.js$$' pages css wafer *.md | while read line; \
	do \
...
docco/%.html: wafer/%.js node_modules/.bin/docco
	mkdir -p docco
	node_modules/.bin/docco -o docco -c docco.css wafer/*.js
